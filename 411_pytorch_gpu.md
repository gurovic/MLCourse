...
**⚠️ Когда GPU быстрее?**
- Большие батчи (batch_size >= 32)
- Большие модели (> 100k параметров)
- Много параллельных операций
- ❌ Для маленьких моделей и батчей GPU может быть медленнее из-за overhead!
...
В Google Colab не забудьте сменить среду выполнения на Графический процессор.
...
## **🚀 Современные оптимизации (PyTorch 2.0+)**

### **torch.compile() - ускорение без изменения кода**
PyTorch 2.0+ включает `torch.compile()` для автоматической оптимизации моделей:
...
# Обучение как обычно - первая итерация будет медленнее (компиляция)
for epoch in range(100):
    output = model(x)
...
**Преимущества:**
- Ускорение 30-200% без изменения кода
- Автоматическая оптимизация графа вычислений
- Fusion операций для уменьшения overhead

### **cuDNN Benchmark - автоподбор алгоритмов**
Для фиксированных размеров входа включите автоматический подбор оптимальных алгоритмов:

```python
# Включить в начале скрипта (до создания модели)
torch.backends.cudnn.benchmark = True

# Это полезно когда:
# - Размер входа не меняется
# - Архитектура модели фиксирована
# Первая итерация будет медленнее (поиск лучшего алгоритма)
```

### **Gradient Checkpointing - экономия памяти**
Для очень больших моделей используйте gradient checkpointing:
...
class MyModel(nn.Module):
    def forward(self, x):
        ...
        # Используем checkpoint - экономит память
        x = checkpoint(self.layer1, x)
        x = checkpoint(self.layer2, x)
        return x
```

**Компромисс:** Экономия памяти за счет ~30% замедления (пересчет активаций при backward).
