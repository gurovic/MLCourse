...
```python
# GPU операции асинхронные относительно CPU
# Это означает, что код на CPU продолжает выполняться, пока GPU считает
x = torch.randn(1000, 1000, device='cuda')
y = torch.randn(1000, 1000, device='cuda')

# Эта операция запускается на GPU
z = x @ y

# Результат доступен сразу (PyTorch ждет при необходимости)
# Но для точного измерения времени нужна синхронизация
...
for epoch in range(1000):
    loss = compute_loss()
    # .item() извлекает значение и автоматически отсоединяет от графа
