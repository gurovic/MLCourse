...
```python
# GPU операции асинхронные по умолчанию
x = torch.randn(1000, 1000, device='cuda')
y = torch.randn(1000, 1000, device='cuda')

# Эта операция запускается, но может не завершиться сразу
z = x @ y

# Синхронизация: ждем завершения всех операций на GPU
torch.cuda.synchronize()

# Теперь результат точно готов
print(z.sum().item())
```
...
for epoch in range(1000):
    loss = compute_loss()
    losses.append(loss.item())  # сохраняем только значение
```
